...
function sinkhorn_nested_distance(tree1::Tree, tree2::Tree, max_nb_children, r, epsilon = 0 )
    """
        ...
        the regularization parameter λ of the entropic wasserstein distance (Sinkhorn's algorithm)
        is adapted to the cost matrix: λ = param_gam*100/max(cost_ij) to avoid numerical instabilities.
    ...
    """
    if epsilon == 0 # set λ = max(cost_ij) / 30 just to avoid numerical instabilities 
        T = height(tree1)+1; # T stage (the root is at height 0)
        D = zeros(Float64, (length(nodes(tree1)), length(nodes(tree2)))); # Alot will remain 0
        # Initialization, computes cost at time T 
        D[1,1] = (abs(tree1.state[1] - tree2.state[1]))^2;
        ...
        for t in T-1:-1:1 
            ...
            for i in nodes1
                for j in nodes2
                    ...
                    children1 = tree1.children[i+1];
                    children2 = tree2.children[j+1];
                    λ =  maximum(C[children1, children2]) / 10;
